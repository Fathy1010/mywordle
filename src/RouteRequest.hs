...
getTarget = do
    seed <- getTimeSeed
    (_, targetWord) <- generateRandomAnswer seed
    return [("target", targetWord)]
...
-- WAS WAS [("feedback","[\"gray\",\"gray\",\"gray\",\"gray\",\"yellow\"]"),("valid","False")]
-- WAS NOW Prelude.undefined
-- NOW [("feedback","[\"gray\",\"gray\",\"gray\",\"gray\",\"yellow\"]"),("valid","False")]
-- >>> getFeedback (JObject [("guess", JString "plate"), ("target", JString "sheep"), ("previousGuess", JNull)])
-- WAS WAS [("feedback","[\"yellow\",\"gray\",\"gray\",\"gray\",\"yellow\"]"),("valid","True")]
-- WAS NOW Prelude.undefined
-- NOW [("feedback","[\"yellow\",\"gray\",\"gray\",\"gray\",\"yellow\"]"),("valid","True")]

-- | Processes a JSON input containing a guess, target, and optional previous guess.
--   Based on the criteria and validation, it returns feedback and the validity of the guess.
getFeedback :: JsonValue -> IO [(String, String)]
getFeedback (JObject kv) = do
    let guess = getString $ getKey "guess" (JObject kv)
        target = getString $ getKey "target" (JObject kv)
        prevGuess = getOptionalString $ getKey "previousGuess" (JObject kv)

    validGuess <- checkValid guess
    let feedback = map show $ makeFeedback guess target
        valid = if validGuess && ensureCriteria prevGuess guess target then "True" else "False"

    return [("feedback", show feedback), ("valid", valid)]
getFeedback _ = error "Invalid JSON input for getFeedback"
...
-- WAS WAS Just "HTTP/1.1 200 OK\r\nContent-Type: ...\r\n\r\n{\"target\":\"...\"}\r\n"
-- WAS NOW Prelude.undefined
-- NOW Just "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: 18\r\n\r\n{\"target\":\"dying\"}\r\n"
--
...
routeRequest (req, site, args) = case (req, site) of
    ...
    ("POST", "/api/makeGuess") -> do
        feedback <- getFeedback args
        return $ Just $ createHttpResponseFromPairs feedback
    _ -> return Nothing
